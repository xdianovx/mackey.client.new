...
const checkoutRef = ref({
  ...
  profile_client_address: {
    flat: adreses?.value[0]?.flat || "",
    floor: adreses?.value[0]?.floor || "",
    house: adreses?.value[0]?.house || "",
    index: adreses?.value[0]?.index || "",
    street: adreses?.value[0]?.street || "",
    entrance: adreses?.value[0]?.entrance || "",
    locality: adreses?.value[0]?.locality || "",
  },
...
<template>
  ...
  >
    <div class="left">
      <div class="container">
        ...
        <div class="adreses-section" v-if="checkoutRef.delivery_method_id != 1">
          ...
          <UserAdresses
            v-if="adreses.length > 0"
            :data="adreses"
            v-model:checkedUserAddress="checkoutRef.profile_client_address_id"
          />

          <div class="mt-4" v-else>
            <p>
              Что бы совершить заказ, неодходимо добавить адрес в личном
              кабинете
            </p>
    ...
    <WidgetsCheckoutDrawer
      ...
      :valid="
        checkoutRef.profile_client_address_id ||
